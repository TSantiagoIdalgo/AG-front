...
const CartMainResume: React.FC<ICartMainResumeProps> = ({
  ...
  const {callMutation, isPending} = useMutation<{ id: string }>(CART_ENDPOINT.POST.createCheckoutSession(), {
    method: "POST",
  });
  ...
  return (
    <article className={Style.cart_page_right}>
      ...
      <div className={Style.cart_summary}>
        ...
        <Button disabled={isCartEmpty || isPending} onClick={onCreateCheckout} text="Proceder con el pago" type="button"
          style={{width: '100%'}}/>
