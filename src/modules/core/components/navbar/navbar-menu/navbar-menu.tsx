...
const LinkComponent = ({img, span, to}: ILinkComponent) => {
  ...
  const colorHover = {Nintendo: '#ff151f', PC: '#ff5400', PlayStation: '#0970d8', Xbox: '#107c10'};
  const bgColor = location.search.includes(span) ? colorHover[span] : 'transparent';
  return (
    <div className={Style.navbar_menu_products_nav}>
      <NavLink to={to} className={Style.navbar_menu_products_access} onClick={() => window.document.title = span}
    ...
    <header className={Style.navbar_menu}>
      ...
      <div className={isScrolling ? Style.navbar_menu_products_scroll : Style.navbar_menu_products}>
        ...
        <div className={Style.navbar_menu_products_links}>
          ...
          <LinkComponent img={PlayStationLogo} span='PlayStation' to="/catalogue?system=PlayStation"/>
          <LinkComponent img={XboxLogo} span='Xbox' to="/catalogue?system=Xbox"/>
          <LinkComponent img={SwitchLogo} span='Nintendo' to="/catalogue?system=Nintendo"/>
          <LandingNavbarSearch/>
        </div>
      </div>
    </header>
